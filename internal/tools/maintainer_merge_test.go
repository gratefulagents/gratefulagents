...
func TestMergePullRequestGateAndSafetyChecks(t *testing.T) {
	...
	approved := `{"state":"OPEN","isDraft":false,"mergeable":"MERGEABLE","reviewDecision":"APPROVED","url":"https://github.com/octo/widgets/pull/7","headRefName":"feature","headRefOid":"abc123"}`
	checksKey := "api repos/{owner}/{repo}/commits/abc123/check-runs --paginate"
	statusKey := "api repos/{owner}/{repo}/commits/abc123/status"
	postKey := "pr view 7 --json state,mergedAt,url,headRefOid"
	merged := `{"state":"MERGED","mergedAt":"2026-07-21T13:00:00Z","url":"https://github.com/octo/widgets/pull/7","headRefOid":"abc123"}`
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			implementer.Status.Artifacts = &platformv1alpha1.AgentRunArtifacts{PullRequestURLs: []string{maintainerTestPullRequestURL}}
			base, k8sClient, _ := newMaintainerToolBase(t, maintainerRun(), implementer)
			setMaintainerMergeEnabled(t, k8sClient, tc.enabled)
			runner := &maintainerFakeRunner{out: map[string]string{
				viewKey: tc.view, checksKey: tc.checkRuns, statusKey: tc.statuses, postKey: merged,
			}}
			...
			if result.IsError || !strings.Contains(result.Content, "Verified pull request https://github.com/octo/widgets/pull/7 merged") || !strings.Contains(result.Content, "accepted issue scope is delivered") {
				t.Fatalf("result = %#v", result)
			}
			if len(runner.calls) != 5 || runner.calls[1] != checksKey || runner.calls[2] != statusKey || runner.calls[3] != tc.mergeCall || runner.calls[4] != postKey {
				t.Fatalf("gh calls = %v, want checks, statuses, merge %q, then verification", runner.calls, tc.mergeCall)
			}
		})
	}
}

func TestMergePullRequestKeepsQueuedMergeActive(t *testing.T) {
	implementer := fleetRun("implementer", platformv1alpha1.AgentRunPhaseRunning)
	implementer.Status.Artifacts = &platformv1alpha1.AgentRunArtifacts{PullRequestURLs: []string{maintainerTestPullRequestURL}}
	base, k8sClient, _ := newMaintainerToolBase(t, maintainerRun(), implementer)
	setMaintainerMergeEnabled(t, k8sClient, true)
	runner := &maintainerFakeRunner{out: map[string]string{
		"pr view 7 --json state,isDraft,mergeable,reviewDecision,url,headRefName,headRefOid": `{"state":"OPEN","isDraft":false,"mergeable":"MERGEABLE","reviewDecision":"APPROVED","url":"https://github.com/octo/widgets/pull/7","headRefOid":"abc123"}`,
		"api repos/{owner}/{repo}/commits/abc123/check-runs --paginate":                      `{"check_runs":[]}`,
		"api repos/{owner}/{repo}/commits/abc123/status":                                     `{"statuses":[]}`,
		"pr view 7 --json state,mergedAt,url,headRefOid":                                     `{"state":"OPEN","url":"https://github.com/octo/widgets/pull/7","headRefOid":"abc123"}`,
	}}
	tool := &mergePullRequestTool{maintainerToolBase: base, runner: runner}
	result, err := tool.Execute(context.Background(), json.RawMessage(`{"pr_number":7}`), maintainerTestGitRepoDir(t))
	if err != nil {
		t.Fatal(err)
	}
	if result.IsError || !strings.Contains(result.Content, "has not reported it merged yet") || !strings.Contains(result.Content, "Keep the work item active") {
		t.Fatalf("result = %#v", result)
	}
}
