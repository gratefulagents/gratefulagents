...
func (t *ComputerUseTool) Execute(ctx context.Context, raw json.RawMessage, _ string) (Result, error) {
	...
	if in.Action.Kind != "observe" {
		return Result{Content: `{"status":"completed","verified":false,"nextStep":"Observe the approved window to verify the effect before reporting success or taking another action."}`}, nil
	}
