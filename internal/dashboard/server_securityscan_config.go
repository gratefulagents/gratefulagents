...
func securityScanPostScriptsFromProto(pbScripts []*platform.SecurityPostScriptConfig) ([]triggersv1alpha1.SecurityScanPostScript, error) {
	...
	for _, p := range pbScripts {
		...
		default:
			return nil, fmt.Errorf("invalid post-script run_on %q (want all, confirmed, high-and-above, high-and-above-actionable, medium-and-above-actionable, or low-and-above-actionable)", p.GetRunOn())
		}
