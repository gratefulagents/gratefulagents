...
func TestBugBountyAcceptanceGuards(t *testing.T) {
	...
	for _, marker := range []string{"final bounty acceptance gate", "severity is low, medium, high, or critical", "severity alone is never a rejection reason", "complete code trace", "unavailable sandbox validation is inconclusive", "missing evidence", "set status `false_positive` only for `disproved`", "set status `confirmed` for `accepted`", "set status `triaged` when technical validation is not ready", "`accepted_risk` is a human decision recorded from the dashboard; a post-script never sets it."} {
...
func TestPriorArtCheckExcludesKnownAndBotFindableWork(t *testing.T) {
	...
	for _, mapping := range []string{
		...
		"`accepted_risk` is a human decision recorded from the dashboard; a post-script never sets it.",
	} {
