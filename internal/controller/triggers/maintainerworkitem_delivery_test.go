...
func (f *fakeMaintainerDeliveryClient) MergePullRequest(context.Context, string, string, int, string, string) (*github.PullRequestMergeResult, error) {
	f.mergeCalls++
	return &github.PullRequestMergeResult{Merged: github.Bool(f.mergeErr == nil)}, f.mergeErr
}
...
func newMaintainerMergeFixture(t *testing.T) (*GitHubRepositoryReconciler, *triggersv1alpha1.GitHubRepository, *triggersv1alpha1.MaintainerWorkItem, *triggersv1alpha1.MaintainerWorkItemCommand) {
	...
	now := metav1.Now()
	mergeable := true
	repository := &triggersv1alpha1.GitHubRepository{ObjectMeta: metav1.ObjectMeta{Name: "repository", Namespace: "default"}, Spec: triggersv1alpha1.GitHubRepositorySpec{Owner: "octo", Repo: "widgets", Maintainer: &triggersv1alpha1.MaintainerSpec{AllowPullRequestMerge: true}}}
	item := &triggersv1alpha1.MaintainerWorkItem{ObjectMeta: metav1.ObjectMeta{Name: "item", Namespace: "default", UID: "item-uid"}, Spec: triggersv1alpha1.MaintainerWorkItemSpec{RepositoryRef: localRef(repository.Name), IssueNumber: 7}, Status: triggersv1alpha1.MaintainerWorkItemStatus{ProjectionSequence: 3, Readiness: &triggersv1alpha1.MaintainerWorkItemReadiness{ReadyToMerge: true}, PullRequests: []triggersv1alpha1.MaintainerWorkItemPullRequestProjection{{IntentName: "monitor", Repository: "octo/widgets", Number: 11, HeadSHA: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", State: triggersv1alpha1.MaintainerWorkItemPullRequestStateOpen, Mergeable: &mergeable, ReviewDecision: string(triggersv1alpha1.PullRequestReviewDecisionApproved), CheckState: triggersv1alpha1.MaintainerWorkItemCheckStatePassing, Fresh: true, HeadObservedAt: &now, ReviewObservedAt: &now, ChecksObservedAt: &now, StatusesObservedAt: &now}}}}
	command := &triggersv1alpha1.MaintainerWorkItemCommand{ObjectMeta: metav1.ObjectMeta{Name: "merge-command", Namespace: "default"}, Spec: triggersv1alpha1.MaintainerWorkItemCommandSpec{Preconditions: triggersv1alpha1.MaintainerWorkItemCommandPreconditions{WorkItemName: item.Name, WorkItemUID: item.UID}, Type: triggersv1alpha1.MaintainerWorkItemCommandTypeRequestMerge, RequestMerge: &triggersv1alpha1.MaintainerRequestMergeCommand{IssueNumber: 7, Repository: "octo/widgets", PullRequestNumber: 11, ExpectedHeadSHA: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", MergeMethod: triggersv1alpha1.MaintainerWorkItemMergeMethodSquash}}}
...
func TestRequestMergeFailsClosedOnZeroReportedChecks(t *testing.T) {
	...
	githubClient := &fakeMaintainerDeliveryClient{pulls: []*polledPullRequest{{State: "open", MergeableKnown: true, Mergeable: true, HeadSHA: head}}, review: triggersv1alpha1.PullRequestReviewDecisionApproved, checks: polledHeadRollup{HeadSHA: head, State: gitHubRollupNone}, statuses: polledHeadRollup{HeadSHA: head, State: gitHubRollupNone}}
...
func TestRequestMergeRejectsHeadChangedAtPreflight(t *testing.T) {
	reconciler, repository, item, command := newMaintainerMergeFixture(t)
	head := command.Spec.RequestMerge.ExpectedHeadSHA
	githubClient := &fakeMaintainerDeliveryClient{pulls: []*polledPullRequest{{State: "open", MergeableKnown: true, Mergeable: true, HeadSHA: "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"}}, review: triggersv1alpha1.PullRequestReviewDecisionApproved, checks: polledHeadRollup{HeadSHA: head, State: gitHubRollupSuccess, Count: 1}, statuses: polledHeadRollup{HeadSHA: head, State: gitHubRollupNone}}
	if err := reconciler.processMaintainerRequestMerge(context.Background(), repository, command, item, githubClient, true); err != nil {
		t.Fatal(err)
	}
	if phase := commandPhase(t, reconciler, command); phase != triggersv1alpha1.MaintainerWorkItemCommandPhaseRejected || githubClient.mergeCalls != 0 {
...
func TestRequestMergeRejectsBlankReviewDecision(t *testing.T) {
	reconciler, repository, item, command := newMaintainerMergeFixture(t)
	head := command.Spec.RequestMerge.ExpectedHeadSHA
	githubClient := &fakeMaintainerDeliveryClient{pulls: []*polledPullRequest{{State: "open", MergeableKnown: true, Mergeable: true, HeadSHA: head}}, checks: polledHeadRollup{HeadSHA: head, State: gitHubRollupSuccess, Count: 1}}
	if err := reconciler.processMaintainerRequestMerge(context.Background(), repository, command, item, githubClient, true); err != nil {
		t.Fatal(err)
	}
	if phase := commandPhase(t, reconciler, command); phase != triggersv1alpha1.MaintainerWorkItemCommandPhaseRejected || githubClient.mergeCalls != 0 {
...
func TestRequestMergeRejectsClosedUnmergedPullRequest(t *testing.T) {
	reconciler, repository, item, command := newMaintainerMergeFixture(t)
	githubClient := &fakeMaintainerDeliveryClient{pulls: []*polledPullRequest{{State: "closed", HeadSHA: command.Spec.RequestMerge.ExpectedHeadSHA}}}
	if err := reconciler.processMaintainerRequestMerge(context.Background(), repository, command, item, githubClient, true); err != nil {
		t.Fatal(err)
	}
	if phase := commandPhase(t, reconciler, command); phase != triggersv1alpha1.MaintainerWorkItemCommandPhaseRejected || githubClient.mergeCalls != 0 {
...
func TestRequestMergeRejectsBypassCapableActor(t *testing.T) {
	reconciler, repository, item, command := newMaintainerMergeFixture(t)
	head := command.Spec.RequestMerge.ExpectedHeadSHA
	githubClient := &fakeMaintainerDeliveryClient{pulls: []*polledPullRequest{{State: "open", MergeableKnown: true, Mergeable: true, HeadSHA: head}}, unsafePolicy: true}
	if err := reconciler.processMaintainerRequestMerge(context.Background(), repository, command, item, githubClient, true); err != nil {
		t.Fatal(err)
	}
	if phase := commandPhase(t, reconciler, command); phase != triggersv1alpha1.MaintainerWorkItemCommandPhaseRejected || githubClient.mergeCalls != 0 {
...
func TestRequestMergeRecordsOnlyConfirmedExpectedHead(t *testing.T) {
	...
	githubClient := &fakeMaintainerDeliveryClient{pulls: []*polledPullRequest{{State: "open", MergeableKnown: true, Mergeable: true, HeadSHA: head}, {State: "closed", Merged: true, MergedAt: mergedAt, HeadSHA: head}}, review: triggersv1alpha1.PullRequestReviewDecisionApproved, checks: polledHeadRollup{HeadSHA: head, State: gitHubRollupSuccess, Count: 1}, statuses: polledHeadRollup{HeadSHA: head, State: gitHubRollupNone}}
	if err := reconciler.processMaintainerRequestMerge(context.Background(), repository, command, item, githubClient, true); err != nil {
		t.Fatal(err)
	}
...
func TestRequestMergeDoesNotResubmitQueuedMerge(t *testing.T) {
	reconciler, repository, item, command := newMaintainerMergeFixture(t)
	head := command.Spec.RequestMerge.ExpectedHeadSHA
	open := &polledPullRequest{State: "open", MergeableKnown: true, Mergeable: true, HeadSHA: head}
...
func TestFinalizeWorkItemRetriesPartialCloseWithoutLosingAttestation(t *testing.T) {
	...
	now := metav1.Now()
	repository := &triggersv1alpha1.GitHubRepository{ObjectMeta: metav1.ObjectMeta{Name: "repository", Namespace: "default"}, Spec: triggersv1alpha1.GitHubRepositorySpec{Owner: "octo", Repo: "widgets", Maintainer: &triggersv1alpha1.MaintainerSpec{}}}
	scope := &triggersv1alpha1.MaintainerAcceptedScope{Statement: "deliver guarded finalization", AcceptanceCriteria: []string{"verified"}}
	item := &triggersv1alpha1.MaintainerWorkItem{ObjectMeta: metav1.ObjectMeta{Name: "item", Namespace: "default", UID: "item-uid"}, Spec: triggersv1alpha1.MaintainerWorkItemSpec{RepositoryRef: localRef(repository.Name), IssueNumber: 7, AcceptedScope: scope}, Status: triggersv1alpha1.MaintainerWorkItemStatus{ProjectionSequence: 5, PullRequests: []triggersv1alpha1.MaintainerWorkItemPullRequestProjection{{IntentName: "monitor", Repository: "octo/widgets", Number: 11, HeadSHA: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", State: triggersv1alpha1.MaintainerWorkItemPullRequestStateMerged, MergedAt: &now}}, AgentRuns: []triggersv1alpha1.MaintainerWorkItemAgentRunProjection{{Name: "implementer", UID: "run-uid", Role: triggersv1alpha1.MaintainerWorkItemAgentRunRoleImplementer, Phase: string(platformv1alpha1.AgentRunPhasePaused)}}}}
	run := &platformv1alpha1.AgentRun{ObjectMeta: metav1.ObjectMeta{Name: "implementer", Namespace: "default", UID: "run-uid", Labels: map[string]string{triggersv1alpha1.MaintainerWorkItemNameLabelKey: item.Name, triggersv1alpha1.MaintainerWorkItemUIDLabelKey: string(item.UID)}}, Status: platformv1alpha1.AgentRunStatus{Phase: platformv1alpha1.AgentRunPhasePaused}}
	command := &triggersv1alpha1.MaintainerWorkItemCommand{ObjectMeta: metav1.ObjectMeta{Name: "finalize-command", Namespace: "default"}, Spec: triggersv1alpha1.MaintainerWorkItemCommandSpec{Preconditions: triggersv1alpha1.MaintainerWorkItemCommandPreconditions{WorkItemName: item.Name, WorkItemUID: item.UID}, Type: triggersv1alpha1.MaintainerWorkItemCommandTypeFinalizeWorkItem, Finalize: &triggersv1alpha1.MaintainerFinalizeWorkItemCommand{IssueNumber: 7, AcceptedScopeHash: maintainerAcceptedScopeHash(scope), DeliverySummary: "all accepted scope delivered", DeliveryEvidence: "PR octo/widgets#11", ImplementerRunNames: []string{"implementer"}}}}
	...
	issues := &flakyFinalizeGitHub{fakeMaintainerGitHub: &fakeMaintainerGitHub{issue: &github.Issue{State: github.String("open")}}, failClose: 1}
