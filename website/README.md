...
- `src/pages/faq.astro` — FAQ sheet, `FAQPage` structured data
- `src/pages/use-cases/` — use-case index and one sheet per application
- `src/data/faq.ts`, `src/data/usecases.ts` — plain-text marketing copy. It
  feeds both visible page copy and JSON-LD, so it must not contain markup.
...
- Page flow: single-viewport product proof + capability dock → product gallery
  ...
  applications index → questions → final self-hosting CTA
- The hero recording is sized from the width its column receives, not from
  `100svh`. The viewport-height formula it replaced collapsed the stage on a
  short window, which is when the product proof matters most. Clicking (or
  Enter/Space on) the recording opens it full screen.
