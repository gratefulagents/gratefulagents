...
export function CommandPalette({ open, onOpenChange, extraItems = [], runLabels }: CommandPaletteProps) {
  ...
  const navItems: PaletteItem[] = React.useMemo(
    () => [
      ...
      // The security section is four pages deep; without these the palette can
      // only reach its front door, which costs two extra clicks for the two
      // views people actually live in (open critical findings, configurations).
      {
        id: "nav.security-runs",
        group: "Go to",
        label: "Security · Scan runs",
        hint: "Every scan run and the findings it reported",
        icon: <ShieldAlert className="size-4" />,
        keywords: ["security", "scan", "runs", "history", "results"],
        action: () => navigate("/security/runs"),
      },
      {
        id: "nav.security-critical",
        group: "Go to",
        label: "Security · Critical findings",
        hint: "Scan runs filtered to critical severity",
        icon: <ShieldAlert className="size-4" />,
        keywords: ["security", "critical", "severity", "findings", "urgent"],
        action: () => navigate("/security/runs?severity=critical"),
      },
      {
        id: "nav.security-configs",
        group: "Go to",
        label: "Security · Configurations",
        hint: "Scheduled and on-demand scan configurations",
        icon: <ShieldAlert className="size-4" />,
        keywords: ["security", "configuration", "schedule", "scan", "setup"],
        action: () => navigate("/security/configs"),
      },
      {
        id: "nav.security-library",
        group: "Go to",
        label: "Security · Library",
        hint: "Workflows, rankers, post-scripts, policy packs, programs",
        icon: <ShieldAlert className="size-4" />,
        keywords: ["security", "library", "workflow", "policy", "pack", "ranker", "program"],
        action: () => navigate("/security/library"),
      },
      {
        id: "nav.shared",
        group: "Go to",
...
function isActiveRoute(item: PaletteItem, pathname: string): boolean {
  ...
  const map: Record<string, string> = {
    ...
    "nav.security-runs": "/security/runs",
    "nav.security-configs": "/security/configs",
    "nav.security-library": "/security/library",
    "nav.shared": "/shared",
