...
export function ContextUsageBar({
  ...
  const tone = ratio >= 0.9 ? "bg-tone-danger" : ratio >= 0.7 ? "bg-tone-warning" : "bg-tone-running";
  ...
  return (
    <span
      className={cn("flex items-center gap-1.5 font-mono text-2xs tabular-nums text-muted-foreground", className)}
      title={detail}
      role="meter"
      aria-valuemin={0}
      aria-valuemax={100}
    ...
    >
      ...
      <span className="relative h-[5px] w-[72px] overflow-hidden rounded-full bg-muted/70 ring-1 ring-inset ring-border/50">
        <span
          ...
          className={cn(
            "absolute inset-y-0 left-0 rounded-full transition-[width] duration-[var(--dur-slow)] ease-[var(--ease-out-quart)]",
            tone,
          )}
