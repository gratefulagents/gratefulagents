      ...
      >
        ...
        <nav className="mt-3 space-y-4">
          ...
          {filtered.map((group, index) => (
            <SettingsNavSection key={group.heading ?? index} group={group} />
          ))}
        </nav>
      </aside>
...
/**
 * One heading + its nav links. Below `md` the links become a horizontal
 * strip with a hidden scrollbar; on a phone the personal group needs ~524px
 * of the ~358px available, so "Role models" and "Git identity" scrolled out
 * of sight with no cue that they existed. Fade the clipped edge.
 */
function SettingsNavSection({ group }: { group: SettingsNavGroup }) {
  const [navRef, navFadeStyle] = useScrollEdgeFade<HTMLUListElement>();
  return (
    <div>
      {group.heading && (
      ...
      <ul
        ref={navRef}
        style={navFadeStyle}
        className="flex gap-1 overflow-x-auto md:flex-col md:overflow-visible [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
      >
