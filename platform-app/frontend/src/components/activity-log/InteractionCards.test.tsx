import { create } from "@bufbuild/protobuf";
import { cleanup, fireEvent, render, screen } from "@testing-library/react";
import { afterEach, describe, expect, it } from "vitest";

import { ActivityFeed } from "@/components/activity-log/ActivityFeed";
import { PlanCard } from "@/components/activity-log/InteractionCards";
import { ActivityEntrySchema } from "@/rpc/platform/service_pb";

afterEach(() => {
  cleanup();
});

describe("PlanCard", () => {
  it("opens the current plan in the shared plan dialog", () => {
    const use = create(ActivityEntrySchema, {
      type: "tool_use",
      tool: "present_plan",
      inputRaw: JSON.stringify({
        summary: "A concise plan summary",
        actions: [{ id: "approve", label: "Approve" }],
      }),
    });

    render(
      <PlanCard
        use={use}
        planContent={"## Current plan\n\n- Implement the popup action"}
      />,
    );

    expect(screen.getByText("A concise plan summary")).toBeTruthy();
    expect(screen.queryByRole("heading", { name: "Current plan" })).toBeNull();

    fireEvent.click(screen.getByRole("button", { name: "View plan" }));

    expect(screen.getByRole("dialog")).toBeTruthy();
    expect(screen.getByRole("heading", { name: "Plan" })).toBeTruthy();
    expect(screen.getByRole("heading", { name: "Current plan" })).toBeTruthy();
  });

  it("uses the durable current plan only for the latest presentation", () => {
    const oldPresentation = create(ActivityEntrySchema, {
      timestampUnix: 1n,
      type: "tool_use",
      tool: "present_plan",
      toolUseId: "old-plan",
      inputRaw: JSON.stringify({
        summary: "Original summary",
        plan: "## Original plan\n\n- Keep this historical version",
        actions: [{ id: "revise", label: "Revise" }],
      }),
    });
    const latestPresentation = create(ActivityEntrySchema, {
      timestampUnix: 2n,
      type: "tool_use",
      tool: "present_plan",
      toolUseId: "latest-plan",
      inputRaw: JSON.stringify({
        summary: "Revised summary",
        actions: [{ id: "approve", label: "Approve" }],
      }),
    });

    render(
      <ActivityFeed
        entries={[oldPresentation, latestPresentation]}
        isLive={false}
        planContent={"## Revised plan\n\n- Use the latest durable version"}
      />,
    );

    const viewPlanButtons = screen.getAllByRole("button", { name: "View plan" });
    fireEvent.click(viewPlanButtons[0]);
    expect(screen.getByRole("heading", { name: "Original plan" })).toBeTruthy();
    expect(screen.queryByRole("heading", { name: "Revised plan" })).toBeNull();

    fireEvent.click(screen.getByRole("button", { name: "Close" }));
    fireEvent.click(viewPlanButtons[1]);
    expect(screen.getByRole("heading", { name: "Revised plan" })).toBeTruthy();
    expect(screen.queryByRole("heading", { name: "Original plan" })).toBeNull();
  });
});
