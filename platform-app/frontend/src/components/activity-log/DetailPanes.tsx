...
function renderJsonValue(value: unknown, indent: number): ReactNode {
  if (value === null) return <span className="text-tone-warning-fg">null</span>;
  if (typeof value === "boolean")
    return <span className="text-tone-warning-fg">{String(value)}</span>;
  if (typeof value === "number")
    return <span className="text-tone-info-fg">{String(value)}</span>;
  if (typeof value === "string")
    return <span className="text-tone-success-fg">&quot;{value}&quot;</span>;
    ...
    return (
      <>
        ...
        {entries.map(([k, v], i) => (
          <span key={k}>
            {pad}
            <span className="text-tone-purple-fg">&quot;{k}&quot;</span>
            <span className="text-muted-foreground/60">: </span>
...
export function TerminalPane({
  ...
  return (
    <div className="overflow-hidden rounded-md border border-border/60 bg-muted/40">
      <pre className="px-3 py-2 text-xs font-mono whitespace-pre-wrap break-all leading-relaxed text-tone-success-fg">
...
const diffLineStyles: Record<UnifiedDiffLineKind, string> = {
  file: "text-diff-file-fg",
  hunk: "bg-diff-hunk/12 text-diff-hunk-fg",
    ...
    return (
      <div className="space-y-2">
        ...
        {diff ? (
          ...
          <>
            {oldStr !== undefined && (
              <div className="overflow-hidden rounded-md border border-diff-del/20 bg-diff-del/5">
                <ScrollArea className="overflow-auto" style={{ maxHeight: 220 }}>
                  <pre className="px-3 py-2 text-xs font-mono whitespace-pre-wrap break-all leading-relaxed text-diff-del-fg">
                    {oldStr}
                  </pre>
                </ScrollArea>
              </div>
            ...
            {newStr !== undefined && (
              <div className="overflow-hidden rounded-md border border-diff-add/20 bg-diff-add/5">
                <ScrollArea className="overflow-auto" style={{ maxHeight: 220 }}>
                  <pre className="px-3 py-2 text-xs font-mono whitespace-pre-wrap break-all leading-relaxed text-diff-add-fg">
                    {newStr}
                  </pre>
                </ScrollArea>
              </div>
