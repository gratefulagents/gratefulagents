...
import { canCreateResource, canMutateResource, formatProviderModels, parseProviderModels, resourceTabs, type ResourceKind } from "@/components/resources/resource-helpers";
...
export function ResourcePage() {
  ...
  return <div className="space-y-5"><header><h1 className="text-[22px] font-semibold">Resources</h1><p className="text-[13px] text-muted-foreground">Configure reusable building blocks for agents and runs.</p></header>
    ...
    {kind === "skills" ? <SkillsSection /> : kind === "mcp-servers" ? <MCPServersSection /> : <ManagedResources kind={kind} creatable={creatable} mutable={mutable} />}
  </div>;
}

function ManagedResources({ kind, creatable, mutable }: { kind: Exclude<ResourceKind,"skills"|"mcp-servers">; creatable: boolean; mutable: boolean }) {
  ...
  const permissionHint = !mutable
    ? kind === "modes" && creatable
      ? "You can create new mode templates. Only administrators can edit or delete existing templates."
