...
export function parseDesktopRelay(raw: string): DesktopRelayStatus {
  ...
  if (value.mode !== "selected_display") throw new Error("Legacy window protocol is unsupported; update desktop/backend/run and reconnect with selected-display and desktop-input consent");
  if (value.pending !== undefined && value.pending !== null) {
    ...
    if (Object.keys(request).some((key) => !["requestId", "frameId", "action"].includes(key))) throw new Error("Legacy or unknown request fields; reconnect with desktop consent");
    ...
    const allowed: Record<string, string[]> = {
      ...
      type: ["kind", "text"], key: ["kind", "key"], open_url: ["kind", "url"],
    };
...
export async function exchangeDesktopRelay(
  ...
  if (scope.mode !== "selected_display" || !Number.isInteger(scope.displayId) || scope.displayId <= 0) throw new Error("Legacy scope: reconnect with selected-display and desktop-input consent");
  const wireOperation = operation === "attach" ? "attach_desktop" : operation;
  const response = await client.exchangeComputerUse({
    ...
    if (wireOperation === "attach_desktop") throw new Error("Selected-display desktop connection failed. Verify run access, update desktop/backend/run to a compatible version, and reconnect with fresh consent.");
    throw error;
  });
  if (scope.backend !== backendBaseUrl()) throw new Error("Desktop backend changed; grant fresh consent");
  const relay = parseDesktopRelay(response.responseJson);
  if (relay.mode !== "selected_display") throw new Error("Desktop protocol unavailable; update desktop, backend and run, then reconnect");
  return relay;
}
