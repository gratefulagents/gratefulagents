...
Built on [PR #398](https://github.com/gratefulagents/gratefulagents/pull/398): restricted sessions still require the native single-window picker selection token and capture through its retained ScreenCaptureKit filter, with no broad Screen Recording permission. Agent-choice sessions have a separate, explicit Screen Recording permission setup in the connection panel. This broader OS permission is necessary for picker-free discovery; granting it does not authorize a session, change modes, or bypass input approval. Both modes use retained single-window ScreenCaptureKit filters for capture and sharing-stop notifications. Discovery requests all ScreenCaptureKit windows, including desktop and off-screen windows, and merges the all-window Core Graphics inventory. Selection refreshes that inventory before binding the chosen reference. See [Window discovery and availability](COMPUTER_USE_WINDOWS.md) for unavailable metadata, capability fields, and integration requirements.

For window-discovery diagnostics, open **Settings → General → Open logs** in the desktop app (macOS logs are under `~/Library/Logs/dev.gratefulagents.app/`). Retry `list_windows` and search for `computer use window discovery`. The `native_candidates` count is the returned native metadata count after supervisor/unknown-owner exclusions, including unavailable candidates, not the raw ScreenCaptureKit count or the safely bound count. Neither native discovery nor the returned listing has a fixed window-count cap. Zero does not establish that ScreenCaptureKit returned no windows. Subsequent logs include the selectable count and rejected numeric window/process IDs with failure reasons.
...
- `raw`: ScreenCaptureKit `content.windows.count` plus CG entries whose window IDs were not present in ScreenCaptureKit; both queries include off-screen and desktop windows. `inspected`: all visited candidates in that merged inventory, equal to `raw` on a successful discovery.
- `missing_owner`: candidates omitted for absent/invalid owner PID, ScreenCaptureKit candidates retained as unavailable metadata because their owner is absent or disagrees with the CG PID, and non-excluded CG-only metadata candidates.
- `supervisor_pid`, `supervisor_bundle`: omitted supervisor candidates. After checking owner PID validity and supervisor identity, ScreenCaptureKit candidates are checked for owner consistency, `missing_name`, `missing_bundle`, `process_identity_unavailable`, and `bundle_mismatch`, in that order. These identity-failure counters describe unavailable metadata as well as omissions; they are counted per candidate at the first failing check, not per distinct application.
- `invalid_snapshot`: identity-valid candidates whose snapshot failed and which remain as unavailable metadata; `non_frontmost`: an informational subset of `eligible`, counting verified windows that are not their application's AX focused window (not necessarily the globally focused application). Do not add it to the accounting total.
- `eligible`: safely bound windows (`inventory.count`), not the total returned metadata count; `cap_uninspected`: the retained field `raw - inspected`, now zero because there is no native count cap.

The identity/exclusion counters and `invalid_snapshot`, plus `eligible`, sum to `inspected`; `inspected + cap_uninspected = raw`. These counters partition candidates by safe-binding outcome, not by whether metadata was returned: unavailable entries can contribute to both `native_candidates` and a failure counter. Logs do not include window titles, application names, bundle identifiers, target references, or screen content. Safe binding still requires all identity checks. `process_identity_unavailable` means the running application is absent/terminated or its kernel identity cannot be verified; it replaces the former `missing_launch_date` counter.
...
| `snapshot_cg_window_missing` | Target window ID is absent from the all-window CG inventory. |
| `snapshot_cg_window_pid_mismatch` | Target CG window exists but its PID differs from the retained application. |
| `snapshot_ax_window_identity_unavailable` | No AX window with the exact CG ID, PID, window role, and bounds could be resolved (including missing AX permission or window-ID API support). |
...
Exactly one snapshot reason is counted per `invalid_snapshot`; the sum of all `snapshot_*` counters equals `invalid_snapshot`. These are a breakdown, **not additional top-level rejections** when reconciling `inspected`. CG lookup requires the exact target ID and then its expected PID; bounds are checked before AX identity. Window layer and CG ordering are not authorization inputs. For overlapping rectangle predicates, null takes precedence over infinite, then empty, without changing the original rejection condition. Other snapshot callers keep the existing method and do not collect diagnostics. No titles, application names, bundle IDs, geometry, or content are logged by these counters.

For example, `raw=3 supervisor_pid=1 process_identity_unavailable=0 invalid_snapshot=2 non_frontmost=0 eligible=0` has exactly two counts distributed among the snapshot reasons and two unavailable metadata entries, assuming all other counters are zero; it does not by itself establish which snapshot check failed. Callback tests cover that accounting for each reason, mixed reasons, zero/eligible inventories, legacy nonzero `cap_uninspected` fixtures, and missing or non-numeric fields. They do not execute AppKit/CG checks; a native Mac retry remains necessary to identify the actual failure.

Process identity is the exact integer tuple `(PID, start seconds, start microseconds)` from `proc_pidinfo(PROC_PIDTBSDINFO)`, shared by the native picker/discovery/snapshot bridge and Rust input binding via C FFI. `NSRunningApplication.launchDate` is LaunchServices-only and may be nil for otherwise eligible apps; it is not an identity prerequisite. Kernel lookup failures, short responses, mismatched PIDs, exiting processes, and invalid start times fail closed, as do absent running applications. There is no PID-only or floating-point fallback. Bundle matching, Accessibility identity, and session consent checks remain required for selection/capture; input additionally requires an on-screen target. Metadata listing alone does not authorize any of these operations.

Window eligibility is shared by native snapshots and Rust input through `computer_use_window.c`. A legitimate floating/elevated window is eligible just like a layer-zero window, whether focused or unfocused. The bridge dynamically resolves the macOS private `_AXUIElementGetWindow` accessor because public AX APIs do not expose the CG window ID; if the symbol or ID lookup is unavailable, the target is rejected. There is no title, geometry-only, or first-window fallback. This private API is an explicit compatibility limitation requiring a real Mac smoke test on supported OS versions.

The snapshot's historical `frontmost` field means **the exact target is its application's AX focused window**, not first in CG stacking order. An unfocused real window from `AXWindows` is eligible for discovery, selection and consented window-only capture. Native snapshots and Rust bindings retain the exact chosen AX element alongside its CG ID, PID and kernel process start identity; focus changes do not replace that identity or revoke observation. Closed/replaced AX identities fail closed even when a replacement reuses the CG ID and bounds. Focused panels are checked directly even if omitted from `AXWindows`. Unresolvable CG-only overlays cannot become targets; same-bounds windows cannot impersonate the target. Pointer ancestry and retained Rust AX bindings use the same exact-ID/PID/role/bounds check. AX access is therefore required to resolve snapshot identity, including capture; missing or not-yet-initialized AX trees fail closed. Consent, retained capture filters, bundle matching, kernel process start identity, bounds checks, and retained AX equality remain in force.

Discovery, selection and observation never raise or focus a window. Input remains gated by the existing approved-window checks: the user must explicitly focus the exact chosen window before input, obtain a fresh frame and queue fresh approved input. The current `activate` action is guard-only (it does not raise the application or select an unfocused sibling window); keyboard execution activates the approved application only after its queue-time focus checks. No input is redirected to a different focused window.

The agent workflow is `list_windows` → `select_window` with a returned `targetRef` → `observe` → input. Discovery returns selectable and unavailable window metadata with bounded application/title text, tri-state `onScreen`, capability hints, and random session-scoped references—not PIDs or OS window IDs. There is no fixed window-count cap; the overall transport size bound still applies. References are replaced by a fresh listing and retired on selection, pause, or stop. The native implementation retains Accessibility window identity and kernel process start time, rechecks fresh window inventory, and refuses closed, reused, or changed identities. Minimized or other-Space candidates may be listed; safe identity is required for selection and capture, macOS may refuse capture, and off-screen input is rejected. Multiple verified windows of the same application may be discovered and observed; the input engine still requires the exact selected window to be the application's focused window.
...
In selected-window sessions, capture uses `SCScreenshotManager` with the **same retained `SCContentFilter` returned by the picker**, never an ID-based reconstruction. The 15.2 `includedWindows` API must identify exactly one window; its owning application PID/name/bundle and process launch identity are bound natively. The picker excludes supervisor bundle/window IDs and rejects supervisor selections again after callback. This restricted path does not enumerate `SCShareableContent`; agent-choice discovery does. Neither path uses Core Graphics image capture. The restricted path uses CG metadata for live bounds, PID and front-to-back order without a Screen Recording grant; the existing AX/input guards remain in place.
...
### AX/CG window identity regression checks

From `platform-app/tauri/src-tauri`:

```sh
cc -std=c11 -Wall -Wextra -Werror -pthread -Ddlsym=ga_test_dlsym -Itests/support \
  src/computer_use_window.c tests/computer_use_window_test.c \
  -o /tmp/computer-use-window-test
/tmp/computer-use-window-test
/tmp/computer-use-window-test missing-symbol
```

These portable fault-injection tests execute the production C helper against test-only AX/CF fixtures, not an Apple ABI substitute. They cover focused target acceptance independent of CG layer/order, focused panels absent from AXWindows, same-bounds overlays, unfocused binding, focus-independent revalidation, moved/closed/reused window identities, wrong-focus input rejection, wrong PID/ID/role/bounds/types, missing attributes, AX query failures, malformed geometry, and a missing private symbol. Native callback Rust tests cover discovery with unfocused eligible windows and diagnostic accounting. Session policy tests cover selection/observation with independent mocked identity and focus, and permanent identity revocation. Source-contract tests guard the Objective-C and Rust call sites. They do **not** execute the Objective-C snapshot, real Accessibility server, ScreenCaptureKit, or OS input.

Before release on a logged-in Mac with explicit consent and permissions:
- Verify both an ordinary layer-zero window and a legitimate focused nonzero-layer floating window appear in `list_windows`, bind through `select_window`, and support approved observation/input.
- Verify unfocused ordinary and floating windows appear in `list_windows`, bind through `select_window`, and support `observe` without focusing. Change focus to a same-app sibling and another app: observation must retain the chosen target; input to the unfocused target must fail. Explicitly focus the chosen target before fresh approved input.
- Put a same-app overlay/panel ahead of the target (including identical bounds); both verified windows may be advertised and observed, but only the actual AX-focused, ID-matched chosen window may receive input. CG-only overlays must report `snapshot_ax_window_identity_unavailable`.
- Revoke AX permissions, close/recreate the target, and relaunch its process; stale identity/input must fail closed. Confirm missing symbol/AX support rejects rather than restoring layer-order or geometry heuristics.
