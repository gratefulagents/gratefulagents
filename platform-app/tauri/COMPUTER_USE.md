...
For window-discovery diagnostics, open **Settings → General → Open logs** in the desktop app (macOS logs are under `~/Library/Logs/dev.gratefulagents.app/`). Retry `list_windows` and search for `computer use window discovery`. The existing `native_candidates` count is **after native filtering and the native 64-window cap**, not the raw ScreenCaptureKit window count; zero does not establish that ScreenCaptureKit returned no windows. Subsequent logs include the selectable count and rejected numeric window/process IDs with failure reasons.
...
- `invalid_snapshot`: identity-valid windows whose snapshot failed; `non_frontmost`: an informational subset of `eligible`, counting verified windows that are not their application's AX focused window (not necessarily the globally focused application). Do not add it to the rejection total when reconciling `inspected`.
...
The agent workflow is `list_windows` → `select_window` with a returned `targetRef` → `observe` → input. Discovery returns at most 32 eligible windows, bounded application/title text, and random session-scoped references—not PIDs or OS window IDs. References are replaced by a fresh listing and retired on selection, pause, or stop. The native implementation retains Accessibility window identity and kernel process start time, rechecks fresh window inventory, and refuses closed, minimized, reused, or changed identities. Multiple verified windows of the same application may be discovered and observed; the input engine still requires the exact selected window to be the application's focused window.
