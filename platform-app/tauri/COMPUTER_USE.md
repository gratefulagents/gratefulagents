...
- `invalid_snapshot`: identity-valid windows whose snapshot failed; `non_frontmost`: valid AX-bound snapshots that are not their application's AX focused window (not necessarily the globally focused application).
...
These portable fault-injection tests execute the production C helper against test-only AX/CF fixtures, not an Apple ABI substitute. They cover focused target acceptance independent of CG layer/order, focused panels absent from AXWindows, same-bounds overlays, unfocused capture-only windows, wrong PID/ID/role/bounds/types, missing attributes, AX query failures, malformed geometry, and a missing private symbol. The native callback Rust tests cover the replacement snapshot rejection counter. They do **not** execute the Objective-C snapshot, real Accessibility server, ScreenCaptureKit, or OS input.
...
- Verify an unfocused floating window is not advertised; focusing another same-app window invalidates input to the retained target. Already-consented unfocused real windows may still be captured without input.
- Put a same-app overlay/panel ahead of the target (including identical bounds); only the actual AX-focused, ID-matched window may be advertised or receive input. CG-only overlays must report `snapshot_ax_window_identity_unavailable`.
