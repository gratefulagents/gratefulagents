...
For window-discovery diagnostics, open **Settings → General → Open logs** in the desktop app (macOS logs are under `~/Library/Logs/dev.gratefulagents.app/`). Retry `list_windows` and search for `computer use window discovery`. Logs include the native candidate count, selectable count, and rejected numeric window/process IDs with failure reasons. They do not include window titles, application names, target references, or screen content. A zero native count differs from candidates being rejected during identity binding; these diagnostics do not relax the eligibility checks.
