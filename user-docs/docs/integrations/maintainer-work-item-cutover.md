...
`request_merge` binds a command to the current work-item projection sequence and resource version, `owner/repository`, pull-request number, and exact 40-character head SHA. Repository configuration must set `spec.maintainer.allowPullRequestMerge: true`, or opt into `spec.maintainer.fullControl: true`.
...
The `latest` checkpoint is stored as an `AgentRun` annotation, bound to both the immutable maintainer-run UID and `GitHubRepository` UID, and advanced with an optimistic-concurrency patch. It therefore survives worker and tool-runtime reconstruction but not deletion/recreation of either identity. Checkpoints expire after 30 days without a successful wait; omission remains the explicit reset/snapshot operation. Unknown, malformed, stale, expired, and cross-boundary handles fail closed. Encoded waiter-v2 cursors remain accepted and returned in the deprecated `cursor` diagnostic field for at least two minor releases after introduction of `cursor_handle`; maintainers should use `cursor: "latest"` instead.
