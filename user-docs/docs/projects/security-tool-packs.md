...
- blockchain/smart-contract: Aderyn, fixed offline Foundry security tests, Echidna, Mythril, Slither, Halmos, and local-rule Semgrep.

The catalog distinguishes executable entries from catalog-only entries. Catalog-only tools remain visible with a reason but cannot produce an invocation. The `security-tools` image (`Dockerfile.security-tools`) carries checksum-locked Nuclei, Naabu, Aderyn, Foundry, and Echidna binaries on amd64 and arm64 alongside the built-in authorization-matrix and crypto-vector runners, plus complete digest-pinned OCI runtime closures for OWASP ZAP, Schemathesis, SSLyze, Nmap, Zeek, Suricata, Mythril, Slither, Semgrep, and Halmos. Those closures execute as ordinary tools inside an unprivileged Bubblewrap root filesystem; they require neither Docker nor a container socket, and the agent cannot replace their executable or arguments. Nuclei uses the single reviewed template committed under `security-knowledge`; automatic template updates and caller-selected templates are disabled. Slither uses the immutable multi-architecture Trail of Bits toolbox index. Semgrep accepts only a repository-reviewed local `.semgrep.yml`; registry rules and `auto` are rejected. Halmos uses a hash-locked Python/Forge/compiler closure with Z3 4.13.0.0, the first compatible Z3 release that publishes both Linux amd64 and arm64 wheels. Its replay record carries architecture-specific closure digests derived from the Python platform manifest, Forge binary, Slither compiler-root manifest, and both hashed Python lock files rather than mislabeling the Python base digest as the scanner closure.
...
Aderyn and Echidna accept canonical-digest Solidity project directories with media type `application/vnd.gratefulagents.solidity-project.v1+directory`. Echidna 2.3.0 uses an explicit seed, one worker, JSON output, 10,000 test sequences of length 32, and a 5,000-step shrink limit; Slither integration is disabled, and caller-selected RPC/FFI configuration is not accepted. A bounded campaign with no falsified property is evidence only about the staged harness and configured limits.
...
Mythril 0.24.8 accepts one self-contained staged Solidity source file (`application/vnd.gratefulagents.solidity-contract.v1+source`) or one staged hex bytecode file (`application/vnd.gratefulagents.evm-bytecode.v1+hex`). Solidity files with imports are rejected before a run is created because single-file staging cannot preserve their dependency graph; use a self-contained source or compiled bytecode target instead. The Job resolves the extracted file rather than passing its containing directory. Mythril runs in its pinned multi-architecture OCI root with networking unshared, breadth-first search, explicit depth/loop/transaction/solver/execution bounds, JSON output, and `--no-onchain-data`; bytecode uses the fixed `--codefile` mode.

Slither 0.11.3 accepts a digest-verified Solidity project and compiles only from an ephemeral writable copy inside its network-disabled OCI root. Semgrep 1.172.0 accepts a staged project only when it contains a local `.semgrep.yml`, runs with metrics disabled, and emits SARIF. Halmos 0.3.3 accepts Foundry projects with existing symbolic tests and executes in a network-disabled, single-concurrency closure with fixed Z3, loop, width, and depth bounds. Halmos exit code 1 is a counterexample; timeout, stuck, revert-all, and exception states are operationally incomplete and never findings or proofs. Missing pinned compiler/dependency closures remain unsupported rather than passing.
