...
Mythril 0.24.8 accepts one staged Solidity source file (`application/vnd.gratefulagents.solidity-contract.v1+source`) or one staged hex bytecode file (`application/vnd.gratefulagents.evm-bytecode.v1+hex`). The Job resolves the extracted file rather than passing its containing directory. Mythril runs in its pinned multi-architecture OCI root with networking unshared, breadth-first search, explicit depth/loop/transaction/solver/execution bounds, JSON output, and `--no-onchain-data`; bytecode uses the fixed `--codefile` mode.
